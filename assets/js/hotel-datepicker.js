...
HotelDatepicker.prototype.getCancelButtonId = function getCancelButtonId () {
        // Get close button ID
...
HotelDatepicker.prototype.addListeners = function addListeners () {
	...
	if (this.showTopbar) {
		...
		document.getElementById(this.getCancelButtonId()).addEventListener('click', function (evt) { return this$1.cancelDatepicker(evt); });
	}
...
HotelDatepicker.prototype.createDatepickerDomString = function createDatepickerDomString () {
	...
	if (this.showTopbar) {
		...
		html += '<div class="datepicker__topbar">' +
					...
					'<button type="button" id="' + this.getCancelButtonId() + '" class="datepicker__cancel-button">Cancel</button>' +
				'</div>';
	}
...
HotelDatepicker.prototype.cancelDatepicker = function cancelDatepicker () {
        // Close the datepicker
	if (!this.isOpen) {
		return;
	}
	...
	// Create event on close
	var evt = document.createEvent('Event');
	evt.initEvent('afterCancel', true, true);
	this.input.dispatchEvent(evt);
...
HotelDatepicker.prototype.showSelectedInfo = function showSelectedInfo () {
	...
	var cancelButton = document.getElementById(this.getCancelButtonId());

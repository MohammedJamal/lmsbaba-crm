$(document).ready(function(){
    ...
    if($("#po_tds_percentage_existing").val())
    {
        var lowp=$("#lowp").val();
        fn_rander_tds_certificate_view('#tds_certificate_div',lowp);
	...
	$("body").on("click",".pfi_edit_view_confirmation",function(e){
    	...
    	var lid =$(this).attr("data-lid");
        swal({
            title: 'Proforma invoice not available. Do you want to add?',
            text: '',
            type: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#DD6B55',
    ...
    $("body").on("click", ".po_pro_forma_invoice_submit", function(e) {
        ...
        if ($('input[name=proforma_type]:checked').length==0) 
        {
          swal('Oops','Please choose proforma type.','error');
          return false;
        } 
    ...
    $("body").on("click", ".po_invoice_submit", function(e) {
        ...
        {
          swal('Oops','Please choose invoice type.','error');
          return false;
        } 
    ...
    $("body").on("click","input:checkbox[name=po_inv_is_digital_signature_checked]",function(e){
        ...
        if(checked) 
        {
            $("#po_inv_digital_signature_title").html('Name of authorized signatory');
    ...
    $("body").on("click",".open_po_preview",function(e){
        var lowp =$(this).attr("data-lowp");
        fn_open_po_preview_popup(lowp);        
    });
  ...
  */
  ...
  // -----------------------------------------------
    ...
    $('#PoPaymentLedgerModal').on('hidden.bs.modal', function () {
        var po_pb=$("#po_pb").val();
        var po_pr=$("#po_pr").val();
        $("#po_payment_recived_div").text(po_pr);
        $("#po_balance_payment_div").text(po_pb);
        // alert(po_pb+'/'+po_pr)
        // window.location.reload();
    }); 
    ...
    $("body").on("click","#po_update_comment_confirm",function(e){
        e.preventDefault();
        var base_url = $("#base_url").val();
        var ThisObj=$(this);       
        var lowp=$("#lowp").val();
        var description = $("#po_uc_comment").val();
        if (description == '') 
        {
            swal('Oops! Comments should not be blank.');
            return false;
        }
        $("#poUpdateCommentFrm").append('<input type="hidden" name="lowp_id" value="'+lowp+'">');
        
       
        $.ajax({
            url: base_url + "order/create_po_update_comment_ajax",
            data: new FormData($('#poUpdateCommentFrm')[0]),
            cache: false,
            method: 'POST',
            dataType: "html",
            mimeType: "multipart/form-data",
    ...
    $("body").on("change","#po_tds_certificate",function(e){
        var base_url = $("#base_url").val();  
        var lowp=$("#lowp").val();
        var extension=$(this).val().replace(/^.*\./, '');
        if(extension.toLowerCase()!='pdf')
        {
            swal('Oops! TDS Certificate should be pdf file.');
            return false;
        }
        $('#po_tds_certificate_frm').append('<input type="hidden" id="lowp" name="lowp" value="'+lowp+'" />');
        $.ajax({
                url: base_url+"order/po_tds_certificate_upload_ajax/",
                data: new FormData($('#po_tds_certificate_frm')[0]),
                cache: false,
                method: 'POST',
                dataType: "html",
                mimeType: "multipart/form-data",
    ...
    $("body").on("click",".del_po_tds_certificate",function(e){
        var lowp=$(this).attr('data-lowp');
        var base_url = $("#base_url").val();
        swal({
            title: 'Are you sure?',
            ...
            $.ajax({
                url: base_url + "order/del_po_tds_certificate_ajax",
                type: "POST",
                data: {
                    'lowp': lowp,
                },

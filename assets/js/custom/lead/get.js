...
$(document).ready(function(){
  ...
  */
	// NOTE
	///////////////////////////////////////////////////////////
  $("body").on("click",".call_report_details",function(e){    
    // var user_id=$(this).attr("data-userid");
    var user_id='';
    var lead_id=$(this).attr("data-leadid");
    var type='total_call';
    // var is_delete_icon_show='N';
    call_report_view_details(date='',type,user_id,lead_id);
});
  ...
  $("body").on("click",".set_c2c",function(e){
    ...
    var user_id=$(this).attr('data-userid');
    var c2c_settings_id=$(this).attr("data-c2c_settings_id");   
    var data="lid="+lid+"&cust_mobile="+cust_mobile+"&cust_id="+cust_id+"&user_mobile="+user_mobile+"&user_id="+user_id+"&contact_person="+contact_person+"&c2c_settings_id="+c2c_settings_id;
    // alert(data); return false;
    swal({
          title: "",
              ...
              complete: function (){
                  ...
                  success: function(data){
                    result = $.parseJSON(data);           
                      ...
                      if(result.c2c_service_provider_id==1)
                      {
                      ...
                      else
                      {
                        ...
                        if(result.c2c_http_code==200)
                        {
                          swal({
                            title: "Click To Call Response text",
                            text: result.msg,
                            type: "success",
                            confirmButtonText: "ok",
                            allowOutsideClick: "false"
                        ...
                        {
                          swal('Oops!','Unknown Error! Please try again!','error');
                        }
                        
                        // alert(result.c2c_http_code+'/'+result.msg)
                      }
  ...
  // ---------------------------------------------------------------
  // Status change
  $("body").on("click", ".lead_status_change", function(e) {      
      var base_url = $("#base_url").val();
      var c_id = $(this).attr("data-cid");      
      var lid = $(this).attr("data-lid");
      var data = "c_id=" + c_id+"&lid="+lid;
      
      $.ajax({
              url: base_url + "lead/change_lead_status_ajax",
              data: data,
              cache: false,
              method: 'POST',
              dataType: "html",
              ...
              success: function(res) {
                  result = $.parseJSON(res);     
                  $("#common_view_modal_title_md").html('');
  ...
  $("body").on("click", "#lead_status_change_submit", function(e) {
      e.preventDefault();
      var base_url = $("#base_url").val();
      // var flag=0;
      if($("#status_id").val()=='')
      ...
      $.ajax({
          url: base_url + "lead/update_lead_status_ajax",
          data: new FormData($('#lead_status_change_frm')[0]),
          cache: false,
          method: 'POST',
          dataType: "html",
          mimeType: "multipart/form-data",
          ...
          processData: false,
          beforeSend: function(xhr) {
            $('#rander_common_view_modal_md .modal-body').addClass('logo-loader');
          },
          complete: function (){
            $('#rander_common_view_modal_md .modal-body').removeClass('logo-loader');
          }, 
          success: function(data) {
              result = $.parseJSON(data);                      
              if (result.status == 'success') {                  
              ...
              else
              {
                swal({
                  ...
                  $("#lead_status_change_submit").attr("disabled",false);
                });
              }
          }
      });
...
{  
      ...
      var opportunity_stage_filter_type=$("#opportunity_stage_filter_type option:selected").val();
      ...
      $("#filter_opportunity_stage_filter_type").val(opportunity_stage_filter_type);
	...
	$("body").on("click",".lead_filter_reset",function(e){
        ...
        $("#opportunity_stage_filter_type").val($("#opportunity_stage_filter_type option:first").val());
        ...
        $("#filter_opportunity_stage_filter_type").val('all_stage');
      ...
      $.ajax({
          ...
          success: function(data){
            ...
            if(result.status=='success')
            {
              ...
              // var web_whatsapp_url_tmp=encodeURI(web_whatsapp_url);             
              // window.open(web_whatsapp_url);
              let params = 'toolbar=yes,scrollbars=yes,resizable=yes,top=500,left=500,width=4000,height=4000';
              popup = window.open(web_whatsapp_url, "WhatsAppPopup", params);
              popup.focus();
            }   
  ...
  */
  ...
  $("body").on("click", ".edit_customer_view", function(e) {
        ...
        $.ajax({
            ...
            async: true,
            success: function(response) {
              console.log('show edit_customer_view_modal')
                $('#edit_customer_view_rander').html(response);
                $('#edit_customer_view_modal').modal('show')
  ...
  */
  ...
  $('#add_source_modal').on('hide.bs.modal', function (e) {
    if ($('#rander_add_new_lead_view_modal').hasClass('temp-hide')) {
        $('#rander_add_new_lead_view_modal').css('display','block').removeClass('temp-hide');
    }
  })
  $("body").on("click",".add_source_popup",function(e){ 

    //rander_add_new_lead_view_modal
    if($('#rander_add_new_lead_view_modal').css('display') == 'block'){
      $('#rander_add_new_lead_view_modal').css('display','none').addClass('temp-hide');
    }   
  ...
  $('#lead_company_details').on('hide.bs.modal', function (e) {
    if ($('#rander_add_new_lead_view_modal').hasClass('in')) {
      $("#rander_add_new_lead_view_modal").css("display","block");
    }
  })
  $("body").on("click",".view_company_detail",function(e){
    $("#rander_add_new_lead_view_modal").css("display","none");
  ...
  $("body").on("click","#cust_reply_submit_confirm",function(e){   
    ...
    $("body").on("click","#regret_comment_update_confirm",function(e){
     ...
     var lead_regret_reason = $("#lead_regret_reason_id :selected").text();
	   ...
	   // var mark_cc_mail_str = '';
	   $('#general_description').val(description);
     $('#lead_regret_reason').val(lead_regret_reason);
    ...
    // =======================================================
    // facebook
    $("body").on('click', '.fb_close', function (e) {
      e.preventDefault();            
      $("#fb_inner_div").html('');
      $("#fb_inner_div").hide('fast');
    });
    $("body").on('click', '.fb_sync_btn', function (e) {
        e.preventDefault();           
        rander_fb_forms_html();	    		
    });
    $("body").on("click","#update_fb_default_form_confirm",function(e){    
        var click_btn_obj=$(this);            
        var base_URL = $("#base_url").val();        
        var fb_form_id=$('input[name="fb_form_id"]:checked').val();
        
        
        if(fb_form_id=='')
        {               
          swal('Oops!','Please select any form','error');
          return false;
        }     
        var data = "fb_form_id="+fb_form_id;	            
        $.ajax({
          url: base_URL+"lead/update_fb_form_set_ajax",
          data: data,
          cache: false,
          method: 'POST',
          dataType: "html",
          ...
          success:function(res){ 
              ...
              {		
                ...
                $("#fb_inner_div").hide('fast');
                $.ajax({
                  url: base_URL + "cronjobs/force_download_from_fb_ajax",
                  type: "POST",
                  data: {},
                  async: true,
                  beforeSend: function( xhr ) {
    ...
    function rander_fb_forms_html(id,lead_id,nid='')
    {
      var base_URL = $("#base_url").val();            	
      var data="";
      $.ajax({
          url: base_URL+"lead/rander_fb_forms_html",
          data: data,
          cache: false,
          method: 'POST',
          dataType: "html",
        ...
        success: function(data){
            result = $.parseJSON(data);                  
            $("#fb_inner_div").html(result.html);                
            $("#fb_inner_div").show('fast');                
        },
      });	
    }
    // facebook
    ...
    $("body").on("click","#download_from_fb",function(e){          
      var base_url = $("#base_url").val();
      $.ajax({
          url: base_url + "cronjobs/force_download_from_fb_ajax",
          type: "POST",
    ...
    $("body").on("click","#download_from_indiamart",function(e){          
        ...
        $.ajax({
            ...
            success: function(response) {                  
              ...
              {
                if(status=='error_code_429'){
                  swal('Your patience will be highly appreciated!','It seems that you have hit the IndiaMART API more than 1 times in last 5 minutes. Please try again after 5 minutes.','error');
                }
                else{
                  swal('fail',msg,'error');
                }
    ...
    $(document).on("click","#download_leads_csv",function (e){
        ...
        var filter_opportunity_stage_filter_type=$("#filter_opportunity_stage_filter_type").val();
  ...
  $("body").on("click","#lead_title_desc_edit_submit_confirm",function(e){
      ...
      $('form#frmLeadEdit').append('<input type="hidden" name="lead_requirement_text"  value="'+lead_requirement+'" />');
      $.ajax({
  ...
  // ==============================================================
  // source change
  $("body").on("click", ".lead_source_change", function(e) {
      var c_id = $(this).attr("data-cid");
      var currsource = $(this).attr("data-currsource");
      var lid = $(this).attr("data-lid");
      var base_url = $("#base_url").val();
      var data = "c_id=" + c_id+"&currsource="+currsource+"&lid="+lid;
      
      $.ajax({
              url: base_url + "lead/change_source_ajax",
              data: data,
              cache: false,
              method: 'POST',
              dataType: "html",
  ...
  $("body").on("click", "#lead_source_change_submit", function(e) {
        e.preventDefault();
        var base_url = $("#base_url").val();
        // var flag=0;
        if($("#lead_source").val()=='')
        {
          $("#lead_source_error").html("Please select lead source.");
          return false;
          // flag=1;
        }
        else
        {
          $("#lead_source_error").html("");
        }
        

        $.ajax({
            url: base_url + "lead/update_lead_source_ajax",
            data: new FormData($('#lead_source_change_frm')[0]),
            cache: false,
            method: 'POST',
            dataType: "html",
            mimeType: "multipart/form-data",
            ...
            success: function(data) {
                ...
                else
                {
                  ...
                  swal({
                      ...
                      $("#lead_source_change_submit").attr("disabled",false);
                    });
                }
            }
        });
  ...
  $("body").on("click",".closer_date_deal_value_popup",function(e){   
      
      var lead_id=$(this).attr("data-id");
      var closer_date=$(this).attr("data-closer_date");  
      var deal_value=$(this).attr("data-deal_value"); 
      var currency_code=$(this).attr("data-currency_code");
      var is_deal_value_editable=$(this).attr("data-is_deal_value_editable");

      $("#lead_closer_date").val(closer_date);
      $("#lead_deal_value").val(deal_value);
      if(currency_code){
        $("#lead_currency_code").val(currency_code).attr("selected","selected");
      }
      else{
        $("#lead_currency_code option:first").attr('selected','selected');
      }

      if(is_deal_value_editable=='N'){
        // $("#lead_deal_value").attr("readonly",true);
        // $("#lead_currency_code").attr('disabled','disabled');
        $("#lead_deal_value_html").css("display",'none');
      }
      else{
        $("#lead_deal_value_html").css("display",'block');
      }
      
      $("#lead_closer_date_deal_value_submit_confirm").attr("data-id",lead_id);
      $('#lead_closer_date_deal_value_modal').modal({backdrop: 'static',keyboard: false});        
  }); 

  $("body").on("click","#lead_closer_date_deal_value_submit_confirm",function(e){
        var thisObj=$(this);
        var base_url=$("#base_url").val();
        var lead_id=$(this).attr("data-id");          
        var closer_date=$("#lead_closer_date").val();  
        var deal_value=$("#lead_deal_value").val();
        var lead_currency_code=$("#lead_currency_code").val();  

        $('form#changeCloserDateDealValueForm').append('<input type="hidden" name="cddv_lead_id" id="cddv_lead_id" value="'+lead_id+'" />');

        $.ajax({
          url: base_url+"lead/change_closer_date_lead_value_ajax",
          data: new FormData($('#changeCloserDateDealValueForm')[0]),
          cache: false,
          method: 'POST',
          dataType: "html",
          mimeType: "multipart/form-data",
          ...
          success: function(data){
            ...
            if(result.status=='success')
            {
              swal({
                ...
                $("#lead_closer_date_"+lead_id).val(closer_date);
                $("#lead_deal_value_"+lead_id).val(lead_currency_code+' '+deal_value);
                $("#cddv_lead_id").val(''); 

                $("#link_lead_closer_date_"+lead_id).attr("data-closer_date",closer_date);
                $("#link_lead_closer_date_"+lead_id).attr("data-deal_value",deal_value);
                $("#link_lead_closer_date_"+lead_id).attr("data-currency_code",lead_currency_code);

                $("#link_lead_deal_value_"+lead_id).attr("data-closer_date",closer_date);
                $("#link_lead_deal_value_"+lead_id).attr("data-deal_value",deal_value);
                $("#link_lead_deal_value_"+lead_id).attr("data-currency_code",lead_currency_code);


                $('#lead_closer_date_deal_value_modal').modal('hide');
              });
  ...
  // MEETING 
    ...
    $("body").on("click",".meeting_schedule_view_popup",function(e){
      ...
      var lead_id=$(this).attr("data-leadid");
      var c_id=$(this).attr("data-cid");
      meeting_schedule_view_popup(lead_id,c_id);           
    });
    ...
    //close-edit-customer-view-modal
    $("body").on("click","#edit_customer_view_modal .close-edit-customer-view-modal-new",function(e){      
      ...
      if ($('#scheduleMeetingModal').hasClass('temp-hide')) {
        //alert(1);
        $('#scheduleMeetingModal').css('display','block').removeClass('temp-hide');
      }
    });
    $("body").on("change","#meeting_assigned_user_id",function(e){      
      var selected_user=$(this).val();
      var selected_meeting_date=$("#selected_meeting_date").val();
      // alert(selected_meeting_date+'/'+selected_user)
    ...
    $("body").on("click","#add_more_contact_persion_view",function(e){
      ...
      $.ajax({
          ...
          async: true,
          beforeSend: function( xhr ) {$('#scheduleMeetingModal').css('display','none').addClass('temp-hide');},
          complete: function() {},
          success: function(response) {
    ...
    // $('#edit_customer_view_modal').on('hide.bs.modal', function (event) {
    //   $('#scheduleMeetingModal').css('display','block');
    // });
          ...
          success: function(data) {
              ...
              if (result.status == 'success') {                                       
                  swal({
                    ...
                    $('#edit_customer_view_modal').modal('hide');
                    // $('#meeting_with_before_checkin_time').append("<option value='"+ename_obj.val()+"'>"+ename_obj.val()+"</option>");
                    $('#scheduleMeetingModal').css('display','block');
                    meeting_schedule_view_popup(lead_id,customer_id); 
...
// function meeting_schedule_view_popup(lead_id,c_id)
// {   
//   var base_url=$("#base_url").val();
//   var data="lead_id="+lead_id+"&c_id="+c_id;      
//   $.ajax({

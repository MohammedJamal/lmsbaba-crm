$(document).ready(function(){    
    ...
    $("body").on("click","#btnContainer .om_get_view",function(e){
        e.preventDefault();
        $(".om_get_view").removeClass("active");
        $(this).addClass('active');
        var view_type=$(this).attr('data-target');
        $("#om_view_type").val(view_type); 
        load_order();
    });
    ...
    $("body").on("change","#form_id",function(e){
        ...
        var pfi_split_id=$("#pfi_split_id").val();
        if(f_id==''){
    ...
    $("body").on("click",".get_om_detail, .click-open-pop",function(e){
    ...
    var pfi_split_id=ThisObj.attr("data-pfi_split_id");
    $.ajax({
...
$("body").on("click",".untag_assign_user_popup_confirm",function(e){
  ...
  $("body").on("click","#om_stage_form_add_submit",function(e){
    ...
    var om_form_is_mandatory=$("#om_form_is_mandatory").val();        
    var data='om_form_name='+om_form_name+"&om_form_is_mandatory="+om_form_is_mandatory;       
...
// ---------------------------------------------
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    if(result.status=='success'){    
                        ...
                        swal({
                            title: "Success!",
                            text: "The Stage deleted successfully.",
                            type: "success",
  ...
  // ---------------------------------------------
    ...
    $("body").on("click",".change_priority",function(e){
        ...
        $.ajax({
                ...
                success: function(data){
                    ...
                    if(result.status=='success'){
                        // load_order();
                        $("#om_page_number_stage_wise"+sid).val('0');
    ...
    $("body").on("click",".change_lock_status",function(e){
        var base_url=$("#base_url").val();
        var po_pi_wise_stage_tag_id=$(this).attr("data-po_pi_wise_stage_tag_id");
        var sid=$(this).attr('data-stage_id');
        var data = 'po_pi_wise_stage_tag_id='+po_pi_wise_stage_tag_id;
        $.ajax({
                url: base_url+"order_management/change_pi_lock_status_ajax",
                data: data,
                //data: new FormData($('#frmAccount')[0]),
                cache: false,
                method: 'GET',
    ...
    $("body").on("click",".split_order",function(e){
        ...
        var pfi=$(this).attr('data-pfi');
        var po_pi_wise_stage_tag_id=$(this).attr('data-po_pi_wise_stage_tag_id');
        var pfi_split_id=$(this).attr('data-pfi_split_id');              
        var data='lowp='+lowp+'&pfi='+pfi+"&po_pi_wise_stage_tag_id="+po_pi_wise_stage_tag_id+"&pfi_split_id="+pfi_split_id; 
        // alert(data); return false;
        $.ajax({
            url: base_url + "order_management/rander_split_order_view_ajax",
            data: data,
            cache: false,
            method: 'POST',
            dataType: "html", 
            ...
            success: function(data){
                result = $.parseJSON(data);
                // alert(result.msg)
                $('#OmSplitModalBody').html(result.html);
                $('#OmSplitModal').modal({
                    backdrop: 'static',
                    keyboard: false
                });      
    ...
    $("body").on("click",".is_split",function(e){
        var sno=$(this).attr("data-sno");
        if($(this).is(':checked'))
        {
            $("#split_qtn_"+sno).attr("readonly",false);
        }else
        {
            $("#split_qtn_"+sno).attr("readonly",true);
        }
    });
    
    $("body").on("click","#om_split_confirm",function(e){
        e.preventDefault();
        var base_url = $("#base_url").val();
        var p_count=0;
        var p_chk_count=0;
        var p_chk_qtn_count=0;
        var po_pro_forma_invoice_product_id_arr=[];
        var po_pro_forma_invoice_product_id_checked_arr=[];
        var p_q_existing_arr=[];
        var p_q_arr=[];
        var p_q_checked_arr=[];
        var pfi=$(this).attr("data-pfi");
        var pfi_split_id=$(this).attr("data-pfi_split_id");
        ...
        var total_cnt=$('input.is_split').length;
        var total_zero_checked_cnt=0;

        $('.is_split').each(function (index, obj) {
            var sno=$(this).attr('data-sno');
            var po_pro_forma_invoice_product_id_tmp=$(this).attr("data-po_pro_forma_invoice_product_id");
            po_pro_forma_invoice_product_id_arr.push(po_pro_forma_invoice_product_id_tmp);

            // var pid_tmp=$(this).attr("data-p_id");
            var q_existing_tmp=parseFloat($("#existing_qtn_"+sno).val());
            p_q_existing_arr.push(po_pro_forma_invoice_product_id_tmp+'~'+q_existing_tmp);

            var q_tmp=$("#split_qtn_"+sno).val();
            p_q_arr.push(po_pro_forma_invoice_product_id_tmp+'~'+q_tmp);

            if (this.checked === true) {
                p_chk_count++;
                p_chk_qtn_count++;
                if($("#split_qtn_"+sno).val()==0 || $("#split_qtn_"+sno).val()==''){ 
                    p_chk_qtn_count=0;                                       
                    return false;
                }
                var po_pro_forma_invoice_product_id_checked_tmp=$(this).attr("data-po_pro_forma_invoice_product_id");
                po_pro_forma_invoice_product_id_checked_arr.push(po_pro_forma_invoice_product_id_checked_tmp);

                // var pid_checked_tmp=$(this).attr("data-p_id");
                var q_checked_tmp=$("#split_qtn_"+sno).val();
                p_q_checked_arr.push(po_pro_forma_invoice_product_id_tmp+'~'+q_checked_tmp);

                if(parseFloat($("#split_qtn_"+sno).val())==q_existing_tmp){
                    total_zero_checked_cnt++;
                }
        ...
        if(p_chk_count==0){
            swal("Oops!", 'Please check product to split.', "error");
            return false;
        }
        if(p_chk_qtn_count==0){
            swal("Oops!", 'Split Quantity should not be blank.', "error");
            return false;
        }
        if(total_cnt>1){
            if(total_cnt==total_zero_checked_cnt){
                swal("Oops!", 'Split Order Quantity should not be greater than or equal to Order Quantity.', "error");            
                $("#split_qtn_"+sno).val('0');
                return false; 
            }
        }
        // alert(total_cnt+'/'+total_zero_checked_cnt)
        // return false;
        // alert('ALl:'+p_q_arr.join(",")+'/Checked:'+p_q_checked_arr.join(",")+'///Existing:'+p_q_existing_arr.join(",")); return false;
        $.ajax({
            url: base_url + "order_management/save_split_order_ajax",
            type: "POST",
            data: {
                'lowp': lowp,
                'pfi': pfi,
                'pfi_split_id':pfi_split_id,
                'po_pro_forma_invoice_product_ids': po_pro_forma_invoice_product_id_arr.join(","),
                'po_pro_forma_invoice_product_ids_checked': po_pro_forma_invoice_product_id_checked_arr.join(","),
                'p_q_ids':p_q_arr.join(","),
                'p_q_checked':p_q_checked_arr.join(","),
                'p_q_existing_arr':p_q_existing_arr.join(",")
            },
            async: false,
            dataType: "html",
            beforeSend: function( xhr ) {
    ...
    $("body").on("focusout",".split_qtn",function(e){        
        var sno=$(this).attr("data-sno");
        var qtn=parseFloat($(this).val());
        var existing_qtn=parseFloat($("#existing_qtn_"+sno).val());
        if($('input.is_split').length=='1'){
            if(qtn>=existing_qtn){
                swal("Oops!", 'Split Order Quantity should not be greater than or equal to Order Quantity.', "error");            
                $("#split_qtn_"+sno).val('0');
                return false;
            ...
            if(qtn>existing_qtn){
                swal("Oops!", 'Split Order Quantity should not be greater than Order Quantity.', "error");            
                $("#split_qtn_"+sno).val('0');
                return false;
            }
            else{}
    ...
    $("body").on("click",".is_split",function(e){   
        ...
        $('.is_split').each(function (index, obj) {
            var sno=$(this).attr("data-sno");
            var qtn=parseFloat($("#split_qtn_"+sno).val());
            var existing_qtn=parseFloat($("#existing_qtn_"+sno).val());           
        ...
        if(flag==true){
            $("#om_split_confirm").show();
        }
        else{
            $("#om_split_confirm").hide();
        }
    });

    $("body").on("click","#view_deliverables",function(e){
        ...
        $.ajax({
          url: base_url + "order_management/rander_deliverables_view_ajax",
          type: "POST",
          data: {
              'lowp': lowp,
              'pfi_split_id':pfi_split_id
    ...
    $("body").on("click",".rander_stage_wise_listing_popup",function(e){
        var base_url=$("#base_url").val();
        var stage_id=$(this).attr("data-stage_id");
        var stage_name=$(this).attr("data-stage_name");
        $.ajax({
            url: base_url + "order_management/rander_stage_wise_listing_view_ajax",
            type: "POST",
            data: {
                'stage_id': stage_id,
            ...
            success: function(data) {
                result = $.parseJSON(data);                
                  ...
                  $("#common_view_modal_title_full").text(result.title);
                  $('#rander_common_view_modal_html_full').html(result.html);
                  $('#rander_common_view_modal_full').modal({
                      backdrop: 'static',
                      keyboard: false
                  });
    ...
    $(document).on('click', '.btn-status-change', function (e) {
        ...
        $.ajax({
                url: base_URL+"order_management/change_is_mandatory",
                data: data,
                //data: new FormData($('#frmAccount')[0]),
                cache: false,
                method: 'POST',
    ...
    $('#OmDetailModal').on('hide.bs.modal', function (e) {
        load_order();
    });

    $("body").on("click","#om_update_comment_confirm",function(e){
        e.preventDefault();
        var base_url = $("#base_url").val();
        var ThisObj=$(this);       
        var proforma_invoice_id=$("#proforma_invoice_id").val();
        var description = $("#om_comment").val();
        if (description == '') 
        {
            swal('Oops! Comments should not be blank.');
            return false;
        }
        $("#omUpdateCommentFrm").append('<input type="hidden" name="proforma_invoice_id" value="'+proforma_invoice_id+'">');
        
       
        $.ajax({
            url: base_url + "order_management/create_om_update_comment_ajax",
            data: new FormData($('#omUpdateCommentFrm')[0]),
            cache: false,
            method: 'POST',
            dataType: "html",
            mimeType: "multipart/form-data",
             ...
             success: function(data) {
                ...
                if (result.status == 'success') 
                {
                    ...
                    function() {
                        ...
                        $("#om_comment").val('');
                        fn_lead_order_history(proforma_invoice_id);
                        // $("#po_uc_file").val('');
    ...
    // ==================================================================================
    // Permission
    $("body").on("click",".tag_permission_link_wise_assign_user_popup",function(e){
        var base_url=$("#base_url").val(); 
        var ThisObj=$(this);
        var permission_link_id=ThisObj.attr("data-permission_link_id");       
        //   alert(permission_link_id); return false;
        $.ajax({
          url: base_url + "order_management/tag_permission_link_wise_assign_user_view_rander_ajax",
          type: "POST",
          data: {
              'permission_link_id': permission_link_id
          },
          async: true,
          success: function(response) {
              $("#common_view_modal_title_md").text("Assign User");
      ...
      $("body").on("click","#tag_permission_link_wise_assign_user_popup_confirm",function(e){
        var base_url = $("#base_url").val();        
        var user_id=$("#om_link_assign_user_id_id").val();
        var permission_link_id=$(this).attr("data-permission_link_id");        
        var data = 'permission_link_id='+permission_link_id+"&user_id="+user_id;  
        ...
        // alert(data); return false;
        $.ajax({
            url: base_url+"order_management/tag_assign_user_to_link_update",
            data: data,                    
            cache: false,
            method: 'GET',
            dataType: "html",                   
            ...
            success: function(data){
                ...
                {
                    $("#rander_common_view_modal_md").modal('hide');
                    load_om_user_wise_permission_view(); 
      ...
      $("body").on("click",".untag_permission_link_wise_assign_user_popup",function(e){
        var base_url=$("#base_url").val(); 
        var ThisObj=$(this);
        var permission_link_id=ThisObj.attr("data-permission_link_id");       
          //alert(permission_link_id);// return false;
        $.ajax({
          url: base_url + "order_management/untag_permission_link_wise_assign_user_view_rander_ajax",
          type: "POST",
          data: {
              'permission_link_id': permission_link_id
          },
    ...
    $("body").on("click",".untag_permission_link_wise_assign_user_popup_confirm",function(e){
        ...
        // alert(data); return false;
        $.ajax({
            url: base_url+"order_management/untag_assign_user_to_link_update",
            data: data,                    
            cache: false,
            method: 'GET',
            dataType: "html",                   
...
function load_order(stage_id='')
{
    if(stage_id){
        load_order_stage_wise(stage_id);
    }
    // else{
        var base_URL=$("#base_url").val();  
        var view_type=$("#om_view_type").val();
        var page=($("#om_page_number").val())?$("#om_page_number").val():'';   
        var data = "view_type="+view_type+"&page="+page;
        //alert(data);// return false;
        $.ajax({
            ...
            success:function(res){ 
                ...
                $(".lock_error").sortable({
                    cancel: ".unsortable",
                    stop: function( event, ui) {            
                        swal('Oops','Please unlock the order to change the stage.','error');           
                    }
                });   
                // Sortable
                // ======================================================================================
                $(".om_page_number_stage_wise").each(function() {
                    var stage_id=$(this).attr('data-stage_id');
                    load_order_stage_wise(stage_id);
                });
            },
...
function load_order_stage_wise(sid)
{
    //alert(wscroll)
    var base_URL=$("#base_url").val();  
    var view_type=$("#om_view_type").val();
    var page=($("#om_page_number_stage_wise"+sid).val())?$("#om_page_number_stage_wise"+sid).val():'';   
    var data = "view_type="+view_type+"&page="+page+"&sid="+sid;
    //   alert(data); //return false;
    $.ajax({
        url: base_URL+"order_management/rander_orders_stage_wise_ajax/",
        data: data,
        cache: false,
        method: 'GET',
        dataType: "html",
        ...
        success:function(res){ 
            result = $.parseJSON(res);
            // alert(result.msg)
            // if(result.is_pagination_active=='N'){
            //     $("#is_pagination_active"+sid).val('N');
            // }
            $("#is_pagination_active"+sid).val(result.is_pagination_active);
            $("#om_page_number_stage_wise"+sid).val(result.page_number);
            // alert(page)
            if(page==0){
                $("#sortable"+sid).html(result.html);
            }
            else{
                $("#sortable"+sid).append(result.html);
            }
...
// ---------------------------------------------
// ---------------------------------------------
function load_om_user_wise_permission_view()
{       
    var base_URL=$("#base_url").val();
    var data = "";        
    $.ajax({
        url: base_URL+"order_management/rander_om_user_wise_permission_ajax/",
        data: data,
        cache: false,
        method: 'GET',
        dataType: "html",
        ...
        success:function(res){ 
            result = $.parseJSON(res);           
            // $("body, html").animate({ scrollTop: 500 }, "slow");               
            $("#om_user_wise_permission_tcontent").html(result.table);            
        },
        complete: function(){
        //removeLoader();
        },
...
function fn_update_pi_stage(pi_id,current_stage_id,prev_stage_id,pfi_split_id='')
{   
    
        var base_url = $("#base_url").val(); 
        var data = 'pi_id='+pi_id+"&current_stage_id="+current_stage_id+"&prev_stage_id="+prev_stage_id+"&pfi_split_id="+pfi_split_id; 
        // alert(data)
        $.ajax({

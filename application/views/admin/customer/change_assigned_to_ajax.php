    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <select class="custom-select select2" name="assigned_to" id="assigned_to" required>
                ...
                {
                    ?>
                    <option value="<?php echo $user_data['id']?>" <?php if($user_data['id']==$currassigned_to){ ?> selected="selected" <?php } ?>><?php echo $user_data['name'].' (Emp. ID: '.$user_data['id'].')';?></option>                    
                    <?php
                }
                ?>    
            </select>
        ...
        <div class="col-md-12">
            ...
            <select class="custom-select select2" name="observer" id="observer" required>
                ...
                {
                    ?>
                    <option value="<?php echo $user_data['id']?>" <?php if($user_data['id']==$assigned_observer){ ?> selected="selected" <?php } ?>><?php echo $user_data['name'].' (Emp. ID: '.$user_data['id'].')';?></option>                    
                    <?php
                }
                ?>    
            </select>
        ...
        <div class="col-md-12 text-right">
            <a href="javascript:;" class="btn btn-primary text-right" id="company_assigne_change_submit">Save</a>
        </div>
    </div>
</form>

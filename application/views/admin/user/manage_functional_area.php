...
<html lang="en">   
  ...
  <body>
    ...
    <div class="card process-sec">
      <?php /* ?>
      <div class=""> 
          <h5 class="lead_board col-md-8">Functional Area List</h5>
        <?php  if(is_method_available('user','add_functional_area')==TRUE){ ?>
        <div class="col-md-4"><a class="pull-right" href="<?php echo base_url();?><?php echo $this->session->userdata['admin_session_data']['lms_url']; ?>/user/add_functional_area"><button type="button" class="btn btn-primary m-r-xs m-b-xs btn-round-shadow mb-0 mt-14"> Add Functional Area
                </button></a>
        </div>
        <?php } ?>
      </div>
      ...
      <div class="col-md-6">
        <div class="card-block">
            <div class="no-more-tables">
              <h3 class="panel-title text-light"><i class="fa fa-list fa-fw text-sm"></i> Manage <strong> Functional Areas</strong></h3><br>
              <?php
              if($rows)
              {       
              ?>
              <table class="table table-bordered table-striped m-b-0 th_color" id="userList">
                ...
                <tr>
                  ...
                  <td class="text-center">
                    <?php  if(is_method_available('user','edit_functional_area')==TRUE){ ?>
                    <a href="<?php echo base_url()?><?php echo $this->session->userdata['admin_session_data']['lms_url']; ?>/user/manage_functional_area/<?php echo $row->id?>"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;
                    <?php }else{ ?>
                    ...
                    <?php } ?>
                    <?php  if(is_method_available('user','delete_functional_area')==TRUE){ ?>
      ...
      <div class="col-md-6">
        <div class="card-block">
          <h3 class="panel-title text-light"><i class="fa fa-list fa-fw text-sm"></i> <?php echo $label; ?> <strong>Functional Area</strong></h3><br>
...
function confirm_delete(id) {
    swal({
      ...
      window.location.href="<?php echo base_url()?><?php echo $this->session->userdata['admin_session_data']['lms_url']; ?>/user/delete_functional_area/"+id;
    });
    return false;
  }

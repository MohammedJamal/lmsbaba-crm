              ...
              <b>Not Logged in:</b> <?php echo (($row['not_logged_day']!='')?$row['not_logged_day'].' Days':''); ?>
                ...
                if(is_array($service_call_type) && count($service_call_type)>0){
                    ...
                    <i class="fa fa-phone-square" aria-hidden="true"></i> <?php echo date_db_format_to_display_format($row['d7_actual_call_done_datetime']); ?></span>
                      ...
                      if($type['id']==4){ ?>
                        <td>
                        <?php if($row['service_end_day']>60){ 
                        if(trim($row['d15_scheduled_call_datetime'])==''){ echo'NA'; } else { ?>
                        <span class="text-primary" title="Call Schedule Date">
                          <i class="fa fa-clock-o" aria-hidden="true"></i> <?php echo date_db_format_to_display_format($row['d15_scheduled_call_datetime']); ?><br></span>
                          
                        <?php if($row['d15_call_status']==1){ ?>
                          <span class="text-success" title="Call Done Date">
                    <i class="fa fa-phone-square" aria-hidden="true"></i> <?php echo date_db_format_to_display_format($row['d15_actual_call_done_datetime']); ?></span>
                        <?php } else { 
                        
                        if($today==date_display_format_to_db_format($row['d15_scheduled_call_datetime'])){
                          echo'<span class="text-warning" title="CALL TODAY"><i class="fa fa-phone-square" aria-hidden="true"></i> TODAY</span>';
                        } elseif($today>date_display_format_to_db_format($row['d15_scheduled_call_datetime'])){
                          echo'<span class="text-danger" title="CALL PENDING"><i class="fa fa-phone-square" aria-hidden="true"></i> PENDING</span>';
                        } elseif($today<date_display_format_to_db_format($row['d15_scheduled_call_datetime'])){
                          echo'<span class="text-secondary" title="CALL UPCOMING"><i class="fa fa-phone-square" aria-hidden="true"></i> UPCOMING</span>';
                        } ?>

                          <br><a href="JavaScript:void(0)" title="<?php echo $type['name']; ?> Update" class="call_comment_update_row text-info" data-cid="<?php echo $row['client_id']; ?>" data-sid="<?php echo $row['service_order_detail_id']; ?>" data-tid="<?php echo $type['id']; ?>" data-title="<?php echo $type['name']; ?>" data-scid="<?php echo $row['d15_call_id']; ?>"><i class="fa fa-commenting-o" aria-hidden="true"></i></a>
                          |
                        ...
                        <?php } } } else { ?>
                          <span class="text-danger">Service
                          <?php 
                          if($row['service_end_day']==0) echo'end Today';
                          elseif($row['service_end_day']>0) echo'end within '.$row['service_end_day'].' days';
                          else echo 'ended '.$row['service_end_day'].' days ago';
                          ;?> </span>
                          <?php } ?>
                        </td>
                        <?php }
                ...
                <td class="<?php echo $tbl_td_color;?>"><?php echo date_db_format_to_display_format($row['next_fllowup_date']); ?><br><br>

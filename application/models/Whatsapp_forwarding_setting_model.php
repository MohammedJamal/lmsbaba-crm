<?php
class Whatsapp_forwarding_setting_model extends CI_Model
{
	private $client_db = '';
	private $fsas_db = '';
	private $class_name = '';
	...
	{
		if($this->client_db->insert('whatsapp_forwarding_settings',$data))
   		{
	...
	public function Editdata($data,$id)
	{
		...
		if($this->client_db->update('whatsapp_forwarding_settings',$data))
		{			
			//return true;			
		}
		else
	...
	{
		$this->client_db -> where('id', $id);
		$this->client_db -> delete('whatsapp_forwarding_settings');
		return true;
	}
	...
	public function GetDetails($id,$client_info=array())
	{
		...
		$sql="SELECT t1.id,
					...
					t1.is_send_skip_manager,
					t1.send_skip_manager_template_id				
				FROM whatsapp_forwarding_settings AS t1 WHERE t1.id='".$id."' AND t1.is_active='Y'";
		$query = $this->client_db->query($sql,false);        
		// echo $last_query = $this->client_db->last_query();die();
		$return=array();
		if($query){

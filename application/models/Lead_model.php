...
class Lead_model extends CI_Model
{
	...
	public function get_call_history_tagged_customer_list()
    {    	
    	$sql="SELECT t1.id,
			t1.number,
			t1.call_start,
			...
			FROM tbl_call_history_for_lead_tmp AS t1 
			INNER JOIN customer AS t2 ON t1.number=t2.mobile 					
			WHERE t1.status='0' AND t1.is_deleted='N'";
    	$query = $this->client_db->query($sql,false);    	
		if($query){
			return $query->result_array();
		}
	...
	public function get_latest_active_lead($argument=array())
    {    	
    	$active_lead_stages=$argument['active_lead_stages'];
    	$customer_id=$argument['customer_id'];
		if($active_lead_stages!='' && $customer_id>0){
			$sql="SELECT 
    			t1.id,
    			t1.title 
    			FROM lead AS t1 
    			WHERE t1.customer_id='".$customer_id."' AND t1.current_stage_id IN (".$active_lead_stages.") ORDER BY t1.id DESC LIMIT 1";
	...
	public function get_latest_lost_lead($argument=array())
    {    	
    	$lost_lead_stages=$argument['lost_lead_stages'];
    	$customer_id=$argument['customer_id'];
		if($lost_lead_stages!='' && $customer_id>0){
			$sql="SELECT 
    			t1.id,
    			t1.title 
    			FROM lead AS t1 
    			WHERE t1.customer_id='".$customer_id."' AND t1.current_stage_id IN (".$lost_lead_stages.") ORDER BY t1.id DESC LIMIT 1";
	...
	public function get_added_fb_form_list()
    { 
		$sql="SELECT 
			t1.id,
			t1.is_default,
			...
			INNER JOIN fb_form AS t2 ON t1.fb_form_id=t2.form_id ORDER BY t2.form_name";		
		$query = $this->client_db->query($sql,false);    	
		if($query){
			return $query->result_array();
		}

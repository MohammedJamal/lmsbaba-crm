<?php
class Indiamart_setting_model extends CI_Model
{
	private $client_db = '';
	private $fsas_db = '';
	private $class_name = '';
	...
	function AddIndiamartCredentials($data)
	{
		if($this->client_db->insert('indiamart_setting',$data))
   		{
	...
	public function EditIndiamartCredentials($data,$id,$client_info=array())
	{
	...
	{
		$this->client_db->truncate('indiamart_setting');	
		return true;	
	}

	public function DeleteIndiamartCredentials($id)
	{
		$this->client_db->where('id', $id);
		$this->client_db->delete('indiamart_setting');
		return true;
	}

	public function GetIndiamartCredentials($client_info=array())
	{
		if($this->class_name=='cronjobs')
		{
			$config['hostname'] = DB_HOSTNAME;
	...
	public function GetIndiamartCredentialsDetails($id)
	{
		$sql="SELECT 
		id,
		is_old_version,
		...
		assign_to,
		assign_start 
		FROM indiamart_setting WHERE id='".$id."'";
		$query = $this->client_db->query($sql,false);        
		//$last_query = $this->client_db->last_query();die();
		$return=array();
		if($query){
	...
	function AddIndiamartCredentialsDetails($data)
	{
		if($this->client_db->insert('indiamart_assign_rule_details',$data))
   		{
           return $this->client_db->insert_id();
   		}
   		else
	...
	{
		$sql="SELECT id,im_setting_id,assign_rule_id,find_to,assign_to,assign_start FROM indiamart_assign_rule_details 
		WHERE im_setting_id='".$sid."'";
		$query = $this->client_db->query($sql,false);
		if($query){
	...
	public function TruncateIndiamartCredentialsDetails()
	{
		$this->client_db->truncate('indiamart_assign_rule_details');	
		return true;	
	}
	public function DeleteIndiamartCredentialsDetails($id)
	{
		$this->client_db->where('im_setting_id', $id);
		$this->client_db->delete('indiamart_assign_rule_details');
		return true;
	}

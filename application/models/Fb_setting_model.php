<?php
class Fb_setting_model extends CI_Model
{
	private $client_db = '';
	private $fsas_db = '';
	private $class_name = '';
	...
	public function EditWebsiteCredentials($data,$id,$client_info=array())
	{
		...
		if($this->client_db->update('fb_lead_assignment_rule_setting',$data))
		{			
			//return true;		
		}
		else
	...
	{
		$this->client_db->where('id', $id);
		$this->client_db->delete('fb_lead_assignment_rule_setting');
		return true;
	}
	...
	public function GetCredentials($client_info=array())
	{
		...
		$sql="SELECT id,
			...
			assign_rule,
			assign_to,
			assign_start FROM fb_lead_assignment_rule_setting LIMIT 1";
	...
	{
		if($this->client_db->insert('fb_lead_assignment_rule_setting',$data))
   		{
	...
	{
		$sql="SELECT id,lead_assignment_rule_setting_id,assign_rule_id,find_to,assign_to,assign_start FROM fb_lead_assignment_rule_setting_details WHERE lead_assignment_rule_setting_id='".$sid."'";
		$query = $this->client_db->query($sql,false);
		return $query->result_array();
	}

	public function GetWebsiteCredentialsDetails($id)
	{
		$sql="SELECT 
		id,
		account_name,
		...
		assign_to,
		assign_start 
		FROM fb_lead_assignment_rule_setting WHERE id='".$id."'";
		$query = $this->client_db->query($sql,false);        
		//$last_query = $this->client_db->last_query();die();
		$return=array();
		if($query->num_rows())
	...
	public function DeleteWebsiteCredentialsDetails($id)
	{
		$this->client_db->where('lead_assignment_rule_setting_id', $id);
		$this->client_db->delete('fb_lead_assignment_rule_setting_details');
		return true;
	}

	function AddWebsiteCredentialsDetails($data)
	{
		if($this->client_db->insert('fb_lead_assignment_rule_setting_details',$data))
   		{
           return $this->client_db->insert_id();
   		}
   		else
	...
	public function get_rule_wise_assigned_user_id($arr,$client_info=array())
	{
		...
		$web_setting_id=$arr['web_setting_id'];
		...
		$sql="SELECT 
			...
			assign_to,
			assign_start 
			FROM website_assign_rule_details WHERE web_setting_id='".$web_setting_id."' AND assign_rule_id='".$assign_rule."'";
		...
		if($query->num_rows())
		{
			...
			$this->client_db->where('id',$id);
			$this->client_db->update('website_assign_rule_details',$post_data);	

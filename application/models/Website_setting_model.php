...
class Website_setting_model extends CI_Model
{
	...
	{
		$this->client_db->where('id', $id);
		$this->client_db->delete('website_api_setting');
		return true;
	}
	...
	public function GetCredentials($client_info=array())
	{
		...
		$sql="SELECT id,
			...
			assign_rule,
			assign_to,
			assign_start FROM website_api_setting LIMIT 1";
	...
	{
		if($this->client_db->insert('website_api_setting',$data))
   		{
	...
	{
		$sql="SELECT id,web_setting_id,assign_rule_id,find_to,assign_to,assign_start FROM website_assign_rule_details WHERE web_setting_id='".$sid."'";
		$query = $this->client_db->query($sql,false);
	...
	public function GetWebsiteCredentialsDetails($id)
	{
		...
		assign_to,
		assign_start 
		FROM website_api_setting WHERE id='".$id."'";
		$query = $this->client_db->query($sql,false);        
		//$last_query = $this->client_db->last_query();die();
		$return=array();
		if($query->num_rows())
	...
	{
		$this->client_db->where('web_setting_id', $id);
		$this->client_db->delete('website_assign_rule_details');
		return true;
	}

	function AddWebsiteCredentialsDetails($data)
	{
		if($this->client_db->insert('website_assign_rule_details',$data))
   		{
           return $this->client_db->insert_id();
   		}
   		else

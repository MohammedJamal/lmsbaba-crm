<?php
class Exporterindia_setting_model extends CI_Model
{
	private $client_db = '';
	private $fsas_db = '';
	private $class_name = '';
	...
	{
		if($this->client_db->insert('exporterindia_setting',$data))
   		{
	...
	{
		$this->client_db->truncate('exporterindia_setting');	
		return true;	
	}
	...
	{
		$this->client_db->where('id', $id);
		$this->client_db->delete('exporterindia_setting');
		return true;
	}
	...
	public function GetTradeindiaCredentialsDetails($id)
	{
		...
		assign_to,
		assign_start 
		FROM exporterindia_setting WHERE id='".$id."'";
		$query = $this->client_db->query($sql,false);        
		//$last_query = $this->client_db->last_query();die();
		$return=array();
		if($query){
	...
	{
		$sql="SELECT id,ti_setting_id,assign_rule_id,find_to,assign_to,assign_start FROM exporterindia_assign_rule_details 
	...
	public function DeleteTradeindiaCredentialsDetails($id)
	{
		...
		$this->client_db->delete('exporterindia_assign_rule_details');
		return true;
	}

	function AddTradeindiaCredentialsDetails($data)
	{
		if($this->client_db->insert('exporterindia_assign_rule_details',$data))
   		{
           return $this->client_db->insert_id();
   		}
   		else

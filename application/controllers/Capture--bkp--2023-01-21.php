...
class Capture extends CI_Controller {
	...
	{
		// ec8bd783ef9d8f1092461753ab832c7b (noidabpo)
        // 759b74ce43947f5f4c91aeddc3e5bad3 (dev)
		//$data = json_decode(file_get_contents('php://input'), true);
		$data = $_REQUEST;		
		$token=trim($data['token']);
		$lead_id=trim($data['lead_id']);
		$call_datetime=trim($data['call_datetime']);
		$call_status=trim($data['call_status']);
		$talk_time=trim($data['talk_time']);
		...
		// $this->Client_model->create_capture_log($post_data_log);			
		// Cron log add			
		// ===============================================================		
		if($error_count==0)
		{			

...
class Cronjobs extends CI_Controller {
	...
	function get_lead_from_exporterindia_get($client_id='')
	{			
		date_default_timezone_set('Asia/Kolkata');
		$current_datetime=date("Y-m-d");	
		$from_date=date("Y-m-d",strtotime($current_datetime));	
		$to_date=date("Y-m-d",strtotime($current_datetime));
		$arg=array();
		$arg['cronjobs_action']='exporterindia';
		$arg['client_id']=$client_id;
		$client_db_info_list=$this->Client_model->get_all($arg);
		if(count($client_db_info_list))
		{	
			...
			{
				$this->Cronjobs_model->initialise($client_info);
				$tradeindia_credentials=$this->Cronjobs_model->exporterindia_setting_GetCredentials($client_info);				
				if(count($tradeindia_credentials)>0)
				{					
					$this->Cronjobs_model->exporterindia_truncate($client_info);
					foreach($tradeindia_credentials AS $ti)
					{
						$exporterindia_setting_id=$ti['id'];					
						$userid=$ti['userid'];	
						$ti_key=$ti['ti_key'];
						$url = "https://my.exportersindia.com/api-inquiry-detail.php";
						$useragent = 'PHP Client 1.0 (curl) ' . phpversion();
						$post_string="k=".$ti_key."&email=".$userid;
						$url_with_get=$url;
						$ch = curl_init();
						curl_setopt($ch, CURLOPT_URL, $url_with_get);
						curl_setopt($ch, CURLOPT_POSTFIELDS, $post_string);
						...
						$return_arr=json_decode($return_tmp);
						$i=0;
						if(!isset($return_arr->msg))
						{
							if(count($return_arr))
							{
								foreach($return_arr as $row)
								{				
									$sender_email=trim($row->email);
									...
									if($sender_email!='' || $sender_mobile!='')
									{
										...
										$detail_req=nl2br($row->detail_req);
										$detail_req=strip_tags($detail_req,'<br>');
										$post=array(
												'exporterindia_setting_id'=>$exporterindia_setting_id,
												'inq_id'=>$row->inq_id,
										...
										$this->Cronjobs_model->exporterindia_insert($post,$client_info);
									}								
									$i++;				
								}
							}
						...
						{
							// die('Message:'.$return_arr->msg);
						}
					}
				}							
			}			
			$this->get_lead_from_exporterindia_set('');	
		}	
	}

	function get_lead_from_exporterindia_set($client_id='')
	{			
		$arg=array();
		$arg['cronjobs_action']='exporterindia';
		$arg['client_id']=$client_id;
		...
		{	
			foreach($client_db_info_list AS $client_info)
			{
				...
				{			
					foreach($tradeindia_credentials AS $credentials)
					{	
						$get_ti=$this->Cronjobs_model->exporterindia_get_rows($credentials['id'],$client_info);
						if(count($get_ti))
						{
							if($credentials['assign_rule']=='1')
							{
							...
							foreach($get_ti as $ti)
							{
								$cust_email=trim($ti['email']);	
								$cust_mobile=($ti['mobile'])?substr(trim($ti['mobile']), -10):'';	
								$inq_id=$ti['inq_id'];							
								$cust_arr=array(
												'email'=>$cust_email,
												'mobile'=>$cust_mobile,
								...
								$get_customer_decision=$this->Cronjobs_model->customer_get_decision_for_ei($cust_arr,$client_info);
								

								// ------------------------
								// get message 
								$ti_history_data=array('msg'=>$get_customer_decision['msg']);								
								$this->Cronjobs_model->exporterindia_update($ti_history_data,$ti['id'],$client_info);
								// ------------------------
								...
								if($get_customer_decision['status']==TRUE)
								{
									...
									if($ti['country'])
									{										
										$get_country=$this->Cronjobs_model->countries_get_country_by_name($ti['country'],$client_info);
										if($get_country!=false)
										{
											$com_country_id=$get_country->id;
											$com_country_code=$get_country->phonecode;
									...
									$assigned_user_id=isset($tradeindia_assign_to[$assign_start])?$tradeindia_assign_to[$assign_start]:$tradeindia_assign_to[0];
									$exporterindia_setting_id=$ti['exporterindia_setting_id'];								
									$com_contact_person=($ti['name'])?$ti['name']:'Purchase Manager';
									$com_designation='Manager';
									$com_email=$cust_email;
									$com_alternate_email='';
									$com_mobile_country_code=$com_country_code;
									...
									if($ti['city'])
									{										
										$get_city_id=$this->Cronjobs_model->cities_get_city_id_by_name($ti['city'],$client_info);
										if($get_city_id!=false)
										{
											$com_city_id=$get_city_id;
										}
									...
									if($ti['state'])
									{										
										$get_state_id=$this->Cronjobs_model->states_get_state_id_by_name($ti['state'],$client_info);
										if($get_state_id!=false)
										{
											$com_state_id=$get_state_id;
										}
									...
									$com_zip='';
									$com_short_description='';
									$com_source_text='Exporterindia Enquiry';									
									$com_source_text_tmp = str_replace(' ', '', strtolower($com_source_text));										
									$com_source_id=$this->Cronjobs_model->source_get_source_id_by_name($com_source_text_tmp,$client_info);
									if($com_source_id==0)
									{
									...
									$lead_requirement='';
									// $lead_requirement .='<h6><B>Lead Title:</B> '.$ti['subject'].'</h6>';							
									if($ti['product']!='' || $ti['subject']!=''){

										if($ti['product']){
											$lead_requirement .='<B>Product Name:</B> '.$ti['product'].'<BR>';
										}
										else{
											if($ti['subject']){
												$lead_requirement .='<B>Product Name:</B> '.$ti['subject'].'<BR>';
											}
										}										
									}
									$lead_requirement .='<BR>';
									if($ti['detail_req']){
										$lead_requirement .='<B>Buying Requirement:</B><BR>';
										$lead_requirement .=''.$ti['detail_req'].'<BR>';
									}

									$lead_requirement .='<B>Buyer\'s Address:</B>';	
									if($ti['city']){
										$lead_requirement .=' '.$ti['city'].'';
									}
									
									if($ti['state']){
										$lead_requirement .=', '.$ti['state'].'';
									}

									if($ti['country']){
										$lead_requirement .=', '.$ti['country'].'';
									}	
									$lead_requirement .='<BR>';	
									
									if($ti['email']){
										$lead_requirement .='<B>Buyer\'s Email:</B>';
										$lead_requirement .=' '.$ti['email'].'';
										$lead_requirement .='<BR>';								
									}

									if($ti['enq_date']){
										$lead_requirement .='<B>Lead Date:</B>';
										$lead_requirement .=' '.date_db_format_to_display_format($ti['enq_date']).'<BR>';							
									}
									
									$lead_requirement .='<B>Source:</B>';
									$lead_requirement .=''.$com_source_text.'<BR>';

									if($inq_id){
										$lead_requirement .='<B>INQ. id:</B>';
										$lead_requirement .=' '.$inq_id.'<BR>';							
									}
									...
									{			
										...
										$arr=array();
										$arr['assign_rule']=$credentials['assign_rule'];
										$arr['exporterindia_setting_id']=$exporterindia_setting_id;
										$arr['search_keyword']=$search_keyword;
										$assigned_user_id=$this->Cronjobs_model->exporterindia_setting_get_keyword_rule5_wise_assigned_user_id($arr,$client_info);
									}
									else
									{
										...
										$arr=array();
										$arr['assign_rule']=$credentials['assign_rule'];
										$arr['exporterindia_setting_id']=$ti['exporterindia_setting_id'];
										$arr['search_keyword']=$search_keyword;											
										$assigned_user_id=$this->Cronjobs_model->exporterindia_setting_get_rule_wise_assigned_user_id($arr,$client_info);
									}				
									// RULE WISE USER			
									// ---------------------
									...
									if($get_customer_decision['msg']=='no_customer_exist')
									{
										...
										if($credentials['assign_rule']=='1')
										{
											...
											// update to setting table
											$tmpdata=array('assign_start'=>$assign_start);											
											$this->Cronjobs_model->exporterindia_setting_EditCredentials($tmpdata,$exporterindia_setting_id,$client_info);
											// update to setting table
											// ----------------------------------
										}
										// RULE WISE USER
									...
									if($is_blacklist=='N')
									{
										...
										$lead_post_data=array(
														...
														'current_status'=>'WARM',
														'ei_inq_id'=>$ti['inq_id'],
														'ei_setting_id'=>$exporterindia_setting_id
													);
										...
										{										
											...
											$comment_title=NEW_LEAD_CREATE_EI;
											$historydata=array(
																'title'=>$comment_title,
																'lead_id'=>$lead_id,
																'comment'=>addslashes($message),
											...
											if($email_forwarding_setting['is_mail_send']=='Y')
											{
												...
												{
													$com_company_name_tmp=$company['name'];
													$product_tmp=($ti['product'])?$ti['product']:$ti['subject'];
													if($product_tmp!=NULL && $com_contact_person_tmp!='')
													{
														$mail_subject=$com_contact_person_tmp.', Your Enquiry # '.$lead_id.' for '.$product_tmp; 
													}
													else if($product_tmp!=NULL && $com_contact_person_tmp=='')
													{
														$mail_subject='Enquiry # '.$lead_id.' for '.$product_tmp; 
													}
													else if($product_tmp==NULL && $com_contact_person_tmp!='')
													{
														$mail_subject=$com_contact_person_tmp.', Your Enquiry # '.$lead_id; 
													}
													else if($product_tmp==NULL && $com_contact_person_tmp=='')
													{
														$mail_subject='Enquiry # '.$lead_id; 
													}
													else
													{
														$mail_subject='Your Enquiry # '.$lead_id.' has been acknowledged';
				...
				if($exporterindia_credentials>0)
				{
					$msg="Exporterindia credentials available.";
				}
				else
				{
					$msg="Exporterindia credentials not available";
				}
			...
			{		
				...
				{					
					...
					if(count($fb_credentials)>0)
					{
						...
						$fb_long_lived_access_token=$fb_credentials['fb_long_lived_access_token'];
						
						$ch = curl_init();
						curl_setopt($ch, CURLOPT_URL, 'https://graph.facebook.com/v16.0/'.$fb_form_id.'/leads?fields=ad_id%2Cad_name%2Cadset_id%2Cadset_name%2Ccampaign_id%2Ccampaign_name%2Ccreated_time%2Cfield_data%2Cform_id%2Chome_listing%2Cid%2Cplatform%2Cis_organic%2Ccustom_disclaimer_responses%2Cpartner_name%2Cretailer_item_id%2Cvehicle&access_token='.$fb_long_lived_access_token);
						curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
						$result = curl_exec($ch);
						if (curl_errno($ch)) {
							$curl_error='Error:' . curl_error($ch);
							echo 'fail#'.$curl_error; die();
						}
						curl_close($ch);
						$return_arr=json_decode($result);						
						...
						if(!isset($return_arr->error->code)){
							if(count($return_arr->data)){
								...
								foreach($return_arr->data AS $form_data){																			
									$post_data=array(
										'fb_id'=>$form_data->id,
							...
							// echo 'fail#'.$msg_log; die();							
							if($return_arr->error->code=='190'){
								echo 'fail#Your Login Facebook Token has been expired. Please contact your admin to reconnect.'; die();
							}
							else{								
								print_r(str_replace(")","",$return_arr->error->message)); die();
								// echo 'fail#'.$tmp_err; die();
							}							
						}			
					}	
					else
					{
						echo 'fail#Oops! Facebook setting not found!'; die();
					}						
				}									
			}		
		}
		...
		{	
			...
			{	
				...
				{				
					...
					{			
						foreach($fb_leads AS $fb)
						{
								...
								foreach($fb_field_and_keyword_k_v AS $tmp_k=>$tmp_val){
									if($tmp_val=='name'){
										if(isset($fb_lead_field_and_value_k_v[$tmp_k])){
											$cust_name=$fb_lead_field_and_value_k_v[$tmp_k];;
										}										
									...
									if($tmp_val=='email'){
										if(isset($fb_lead_field_and_value_k_v[$tmp_k])){
											$cust_email=$fb_lead_field_and_value_k_v[$tmp_k];;
										}										
									...
									if($tmp_val=='phone_number'){
										if(isset($fb_lead_field_and_value_k_v[$tmp_k])){
											...
											$haystack=$fb_lead_field_and_value_k_v[$tmp_k];
											$needle="+91";
											if (strpos($haystack, $needle) !== false) {
												$cust_country_from_mobile_tmp='India';
											}
										}								
									}
									...
									if($tmp_val=='company_name'){
										if(isset($fb_lead_field_and_value_k_v[$tmp_k])){
											$cust_company_name=$fb_lead_field_and_value_k_v[$tmp_k];;
										}										
									...
									if($tmp_val=='description'){
										if(isset($fb_lead_field_and_value_k_v[$tmp_k])){
											$cust_description=$fb_lead_field_and_value_k_v[$tmp_k];;
										}										
									...
									if($tmp_val=='city'){
										if(isset($fb_lead_field_and_value_k_v[$tmp_k])){
											$cust_city=$fb_lead_field_and_value_k_v[$tmp_k];;
										}										
									...
									if($tmp_val=='state'){
										if(isset($fb_lead_field_and_value_k_v[$tmp_k])){
											$cust_state=$fb_lead_field_and_value_k_v[$tmp_k];;
										}										
									...
									if($tmp_val=='country'){
										if(isset($fb_lead_field_and_value_k_v[$tmp_k])){
											$cust_country=$fb_lead_field_and_value_k_v[$tmp_k];;
										}
									}

									if($tmp_val=='pin'){
										if(isset($fb_lead_field_and_value_k_v[$tmp_k])){
											$cust_pin=$fb_lead_field_and_value_k_v[$tmp_k];;
										}										
							...
							if($get_customer_decision['status']==TRUE)
							{
								...
								{
									if($cust_country_from_mobile_tmp){
										$get_country=$this->Cronjobs_model->countries_get_country_by_name($cust_country_from_mobile_tmp);
										if($get_country!=false)
										{
											$com_country_id=$get_country->id;
											$com_country_code=$get_country->phonecode;
								...
								$com_zip=$cust_pin;
								$com_short_description=$cust_description;
								if($fb['fb_platform'])
								{
					...
					if($lead_inserted_count==0){
						echo 'success#No new facebook lead found to add as lead in our system.';	
					}
					else{
						echo $status.'#'.$msg;
					}							

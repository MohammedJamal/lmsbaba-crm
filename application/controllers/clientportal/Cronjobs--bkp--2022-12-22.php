...
class Cronjobs extends CI_Controller {
			...
			if(count($client_db_info_list))
			{
				...
				{				
					try {
							...
							{	
								$smtp_data=$this->Cronjobs_model->GetSmtpData();								
								// if(count((is_countable($smtp_data)?$smtp_data:[]))){
								if($smtp_data){
									$smtp_data_tmp=$smtp_data;
								}	
								else{
								...
								if(count($get_mail))
								{
									...
									foreach($get_mail as $mail)
									{
										...
										{			
											// $attach_arr_tmp=array();
											// if($mail['attach']){
											// 	$attach_arr_tmp=unserialize($mail['attach']);												
											// }																					
											$m_d['smtp_data']     = $smtp_data_tmp;											
											...
											$i++;
											$this->Cronjobs_model->cronjobs_mail_fire_delete($mail['id']);
	...
	function get_lead_from_indiamart__backup_2022_09_03($client_id='')
	{	
		try {
			...
			if(count($client_db_info_list))
			{
				...
				// ================================================
				// Cron log update
				// $offset_tmp=($get_limit_offset=='0')?'150':'0';
	...
	function get_lead_from_indiamart_by_manual___bkp__2022_09_05($client_id='')
	{	
		if($client_id=='')
		{
			echo 'fail#Parameter is missing.';die();
	...
	function get_lead_from_tradeindia___bkp__2022_09_05($client_id='')
	{	
		date_default_timezone_set('Asia/Kolkata');
		$current_datetime=date("Y-m-d");	
		$from_date=date("Y-m-d",strtotime($current_datetime));	
	...
	function get_lead_from_aajjo__bkp__2022_09_05($client_id='')
	{	
		date_default_timezone_set('Asia/Kolkata');
		$current_datetime=date("Y-m-d");
		$from_date = date("Y-m-d",strtotime("-1 days", strtotime($current_datetime)));
							...
							if(count($return_arr->Records)>0)
							{
								...
								{				
									...
									if($sender_email!='' || $sender_mobile!='')
									{
										...
										// $is_already_exist=$this->Cronjobs_model->aajjo_is_record_already_exist($arr,$client_info);
										...
										// 		$this->Cronjobs_model->aajjo_update($post,$is_already_exist['id'],$client_info);
										// 	}										
										// }									
									}								
									$i++;				

...
function add_lead_ajax()
{
	...
	{		
		...
		$data['currency_list']=$this->Product_model->GetCurrencyList();
		$data['fb_form_count']=$this->lead_model->get_added_fb_form_list();
	...
	function update_auto_tagged_call_to_lead_ajax()
	{
		$is_call_log_updated='N';
		$msg='';
		// update tbl_call_history_for_lead_tmp
		$get_tagged_customer_list=$this->lead_model->get_call_history_tagged_customer_list();		
		if(count($get_tagged_customer_list)){
			$is_call_log_updated='Y';
			$update_by=$this->session->userdata['admin_session_data']['user_id'];
			$date=date("Y-m-d H:i:s");				
			$ip_addr=$_SERVER['REMOTE_ADDR'];		
			$comment_title=LEAD_UPDATE_MANUAL;
			$arg['lost_lead_stages']=$this->lead_model->get_type_wise_lead_stage('LL');
			$arg['active_lead_stages']=$this->lead_model->get_type_wise_lead_stage('AL');			
			foreach($get_tagged_customer_list AS $row){
				$arg['customer_id']=$row['customer_id'];
				$get_latest_active_lead=$this->lead_model->get_latest_active_lead($arg);
				$status_wise_msg='';
				if(count($get_latest_active_lead)){					
					$status_wise_msg="Follow-up call for Lead &quot;".$get_latest_active_lead['title']." (#".$get_latest_active_lead['id'].")&quot;.  call start time: ".datetime_db_format_to_display_format($row['call_start'])." and call end time: ".datetime_db_format_to_display_format($row['call_end']);
			
					$post_data=array(
									'tagged_lead_id'=>$get_latest_active_lead['id'],
									'status'=>'3',
									'status_wise_msg'=>$status_wise_msg
									);
					$r=$this->lead_model->update_sync_call($post_data,$row['id']);
					if($r==true){												
						$historydata=array(
										'title'=>$comment_title,
										'lead_id'=>$get_latest_active_lead['id'],
										'comment'=>addslashes($status_wise_msg),
										'attach_file'=>'',
										'create_date'=>$date,
										'user_id'=>$update_by,
										'ip_address'=>$ip_addr
					...
					$get_latest_lost_lead=$this->lead_model->get_latest_lost_lead($arg);
					if(count($get_latest_lost_lead)){						
						$status_wise_msg="Follow-up call for Lead &quot;".$get_latest_lost_lead['title']." (#".$get_latest_lost_lead['id'].")&quot;.  call start time: ".datetime_db_format_to_display_format($row['call_start'])." and call end time: ".datetime_db_format_to_display_format($row['call_end']);
						$post_data=array(
										'tagged_lead_id'=>$get_latest_lost_lead['id'],
										'status'=>'3',
										'status_wise_msg'=>$status_wise_msg
										);
						$r=$this->lead_model->update_sync_call($post_data,$row['id']);
						if($r==true){							
							$historydata=array(
												'title'=>$comment_title,
												'lead_id'=>$get_latest_lost_lead['id'],
												'comment'=>addslashes($status_wise_msg),
												'attach_file'=>'',
												'create_date'=>$date,
												'user_id'=>$update_by,
		...
		if($is_call_log_updated=='N'){
			$msg="Call Log Lead not found to update.";
		}
	    $data =array (
	       	"status"=>'success',
			"is_call_log_updated"=>$is_call_log_updated,
			"msg"=>$msg
	        );	   
	    $this->output->set_content_type('application/json');
	    echo json_encode($data);
	...
	function rander_fb_forms_html()
    {
		...
		$list=array();	
		$list['rows']=$this->lead_model->get_added_fb_form_list();
		$html = $this->load->view('admin/lead/fb_form_list_view_ajax',$list,TRUE);
        $result['html']=$html;
        echo json_encode($result);
        exit(0);        
	}
	public function update_fb_form_set_ajax()
	{
		$session_data = $this->session->userdata('admin_session_data');
		$user_id = $session_data['user_id'];	
		...
		{			
			...
			$return=$this->Setting_model->EditFormWiseLeadField($post);	
			...
			$return=$this->Setting_model->EditFormWiseLeadField($post,$fb_form_id);			
			if($return)
			{
				$msg='Record successfully updated';
				$status='success';

	...
	function qutation_send_to_buyer_by_mail_ajax()
  	{
		...
		if($list['quotation_data']['quotation']['customer_id']){
			$customer=$this->Customer_model->GetQuotationWiseCustomerData($list['quotation_data']['quotation']['customer_id']);			
		}
		else{
			$customer=array();
		}
 ...
 //   	 	$last_id=$last_id->id+1;
  ...
  function bulk_qutation_send_to_buyer_by_mail_confirm_ajax()
  {
		...
		if(strtolower($_SERVER["REQUEST_METHOD"]) == 'post')
		{
			...
			if($action_type=='add')
			{
				...
				if(count($tmp_prod_list))
				{
					...
					if($quotation_id){
						...
						if($customer_id_in_created_quotation=='0'){
							if($lead_id!='' && $quotation_id!='' && $opportunity_id!=''){
								...
								$this->quotation_model->qutation_delete2($arg);
							}
							$result["status"] = 'fail';
							$result["error_msg"] = 'Something went wrong. Please try again.';
							echo json_encode($result);
					...
					// if($lead_data->customer_id!='' && $lead_data->customer_id>0){
	...
	public function selected_additional_charges_added_ajax()
	{
        ...
        $list['opportunity_id']=$opportunity_id;
        $list['quotation_id']=$quotation_id;
		$list['lead_id']=get_value("lead_id","lead_opportunity","id=".$opportunity_id);
        $list['selected_additional_charges']=$this->Opportunity_model->get_selected_additional_charges($opportunity_id);
        $list['prod_list']=$this->quotation_model->GetQuotationProductList($quotation_id);
        $list['currency_list']=$this->lead_model->GetCurrencyList();
        $list['quotation_data']=$this->quotation_model->GetQuotationData($quotation_id);
	...
	public function rander_quotation_product_list_for_rearrange()
	{	
		$quotation_id=$this->input->post('quotation_id');
		$opportunity_id=$this->input->post('opportunity_id');
		$lead_id=$this->input->post('lead_id');
		...
		$list['prod_list']=$this->quotation_model->GetQuotationProductList($quotation_id);
		$list['selected_additional_charges']=$this->Opportunity_model->get_selected_additional_charges($opportunity_id);    
        $html = $this->load->view('admin/quotation/rander_quotation_product_list_for_rearrange_ajax',$list,TRUE);      

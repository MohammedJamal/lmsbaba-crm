...
class My_pending_calls extends CI_Controller {
	function __construct()
	{
		parent :: __construct();
		is_adminportal_logged_in();
		chk_access_menu_permission(8);
	...
	{	
		...
		$data['service_call_type']=$this->Adminportal_model->get_service_call_type_list('4,5,6');
		$this->load->view('adminmaster/my_pending_calls_view',$data);
	}
	...
	public function get_all_client_wise_service_list_ajax()
	{
		...
		if(strtolower($_SERVER["REQUEST_METHOD"]) == 'post')
		{
			...
			$config['base_url'] ='#';
			$config['total_rows'] = $this->Adminportal_model->get_my_pending_all_calls_list_count($argument);	
			...
			$list['rows']=$this->Adminportal_model->get_my_pending_all_calls_list($argument);
			$list['page']=$page_link;
			$list['page_record_count_info']=$page_record_count_info;
			$list['sl_start']=$tmp_start;
			$list['service_call_type']=$this->Adminportal_model->get_service_call_type_list('4,5,6');
			$html = $this->load->view('adminmaster/my_pending_calls_all_list_view_ajax',$list,TRUE);
			$status_str='success';  
    ...
    public function save_call_comment_update_ajax_BACKUP_23_11_22()
	{
		if(strtolower($_SERVER["REQUEST_METHOD"]) == 'post')
		{
			...
			if($client_id!='' && $client_update_type_id!='' && $activity_text!='' && ($call_done!='' OR $followup_date!=''))
			{
				...
				if($call_done==1){
					...
					$call_type_arr=array(5,6,7);
					if(!in_array($call_type_id, $call_type_arr)){
						$insert_data=array(
							'client_id'=>$client_id,
					...
					if($call_type_id==5 || $call_type_id==6){
						if($call_type_id==5){
							$subsql=" AND service_call_type_id IN(1,2,3,4) AND client_id='".$client_id."' ";
						}

						if($call_type_id==6){
							$subsql=" AND service_call_type_id IN(1,2,3,4) AND service_id='".$service_id."' ";
						}
						
						$subsql.=" AND service_call_type_id IN(1,2,3,4) ";
						...
						if(count($call_list) && is_array($call_list)){
							foreach($call_list AS $calls){
								...
								if($calls['service_call_type_id']==1) {
									$scheduled_call_datetime=$today;
								} elseif($calls['service_call_type_id']==2) {
									$scheduled_call_datetime=date('Y-m-d H:i:s',strtotime("+3 day", strtotime($today)));
								} elseif($calls['service_call_type_id']==3) {
									$scheduled_call_datetime=date('Y-m-d H:i:s',strtotime("+7 day", strtotime($today)));
								} elseif($calls['service_call_type_id']==4) {
									$scheduled_call_datetime=date('Y-m-d H:i:s',strtotime("+15 day", strtotime($today)));
								}
	...
	public function update_service_call_list_xxx()
	{

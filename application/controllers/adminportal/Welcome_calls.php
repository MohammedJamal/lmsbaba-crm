...
class Welcome_calls extends CI_Controller {
	...
	{	
		...
		$data['service_call_type']=$this->Adminportal_model->get_service_call_type_list('1,2,3');
		$this->load->view('adminmaster/welcome_calls_view',$data);
	}
	...
	public function get_all_client_wise_service_list_ajax()
	{
		...
		if(strtolower($_SERVER["REQUEST_METHOD"]) == 'post')
		{
			...
			$config['base_url'] ='#';
			$config['total_rows'] = $this->Adminportal_model->get_all_welcome_calls_list_count($argument);	
			...
			$list['rows']=$this->Adminportal_model->get_all_welcome_calls_list($argument);
			$list['page']=$page_link;
			$list['page_record_count_info']=$page_record_count_info;
			$list['sl_start']=$tmp_start;
			$list['service_call_type']=$this->Adminportal_model->get_service_call_type_list('1,2,3');
			$html = $this->load->view('adminmaster/welcome_calls_all_list_view_ajax',$list,TRUE);
			$status_str='success';  
	...
	public function get_call_log_list_ajax()
	{
		if(strtolower($_SERVER["REQUEST_METHOD"]) == 'post')
		{
			...
			$html = $this->load->view('adminmaster/welcome_calls_log_list_view_ajax',$list,TRUE);
			$status_str='success';  
	        $result["status"] = $status_str;
	        $result["html"]=$html;	  
    ...
    public function get_call_comment_view_ajax()
	{
		if(strtolower($_SERVER["REQUEST_METHOD"]) == 'post')
		{
			...
			{	
				...
				$html=$this->load->view('adminmaster/get_welcome_call_comment_view_popup_ajax',$list,TRUE);					
				$status_str='success';
				$result["status"] = $status_str;	
				$result["html"] = $html;	  
				echo json_encode($result);
		...
		if(strtolower($_SERVER["REQUEST_METHOD"]) == 'post')
		{
			...
			if($client_id!='' && $client_update_type_id!='' && $activity_text!='' && ($call_done!='' OR $followup_date!=''))
			{
				...
				$today=date("Y-m-d H:i:s");
					...
					$activity_text.="<br>Module & Service: ".$module_name." (".$service_name.")";

                    $post_data=array();
                    $post_data['user_id']=$user_id;
                    $post_data['client_id']=$client_id;
                    $post_data['activity_type']=$activity_type;
                    $post_data['client_update_type_id']=$client_update_type_id;
                    $post_data['activity_title']=$activity_title;
                    $post_data['activity_text']=$activity_text;
                    $post_data['ip_address']=$ip_address;
                    if(($call_done==1 && $call_type_id!=3) || $call_done!=1) {
                        $post_data['followup_date']=$followup_date;
                    }
                    $post_data['created_at']=$today;

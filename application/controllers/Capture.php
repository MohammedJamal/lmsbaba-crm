...
class Capture extends CI_Controller {
	...
	public function nimbus_call()
	{
		...
		$call_date=trim($data['call_date']);
		$call_start_time=trim($data['call_start_time']);
		$call_end_time=trim($data['call_end_time']);	
		$call_duration=trim($data['call_duration']);			
		$call_status=trim($data['call_status']);
		$description=trim($data['description']);
		$disposition=trim($data['disposition']);
		$sub_disposition=trim($data['sub_disposition']);	
		$follow_up_date=trim($data['follow_up_date']);	
		$follow_up_time=trim($data['follow_up_time']);
		$call_recording_url=trim($data['call_recording_url']);
		// ---------------------------------------
		// CHECKING
		$error_count=0;
		$msg_error='';
		if($token=='' || $token==NULL)
		{
			...
			if($token=='ec8bd783ef9d8f1092461753ab832c7b') 
			{
		...
		{
			$error_count++;
			$msg_error .='Lead ID is missing...';
		}

		// if($call_datetime=='' || $call_datetime==NULL)
		// {
		// 	$error_count++;
		// 	$msg_error .='Call datetime is missing...';
		// }

		// if($call_status=='' || $call_status==NULL)
		// {
		// 	$error_count++;
		// 	$msg_error .='Call status is missing...';
		// }	
		// else 
		// {
		// 	if($call_status=='Y' || $call_status=='N'){

		// 	}
		// 	else{
		// 		$error_count++;
		// 		$msg_error .='Call status should be either Y or N ...';
		// 	}
		// }	
		
		// if($talk_time=='' || $talk_time==NULL)
		// {
		// 	$error_count++;
		// 	$msg_error .='Total talktime is missing...';
		// }

		// if($description=='' || $description==NULL)
		// {
		// 	$error_count++;
		// 	$msg_error .='Description is missing...';
		// }

		// if($call_recording_url=='' || $call_recording_url==NULL)
		// {
		// 	$error_count++;
		// 	$msg_error .='Call recording URL is missing...';
		// }
		// CHECKING
		// ---------------------------------------
		...
		{			
			if(count($client_info))
			{
				...
				$this->Cronjobs_model->capture_nimbus_data_in_tmp_table($post_data);					
				// 	
				// ===============================================================					
			}
			echo'success';
